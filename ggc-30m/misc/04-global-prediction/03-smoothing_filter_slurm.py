...
def _urls(tile, p, base_url = 'http://192.168.49.40:8333', years=range(2000,2023,2)):
  return [
    f'{base_url}/tmp-gpw/grassland_v1/{tile}/gpw_rf.{p}_30m_m_{y}0101_{y}1231_go_epsg.4326_v21.tif'
    for y in years 
  ]
...
def _processed(tile):
  url = f'http://192.168.49.30:8333/tmp-gpw/grassland_v1/{tile}/gpw_nat.semi.grassland_rf.savgol.diff_p_30m_20000101_20221231_go_epsg.4326_v1.tif'
  r = requests.head(url)
...
ids_fn = str(base_dir.joinpath('gpw_pasture.class_ids_missing.csv'))
...
mask_prefix = 'http://192.168.1.30:8333/gpw/predmask'
